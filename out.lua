...
local function main()
	...
	SettingsWindow.Init = function()
		...
		local decompilerOption = {"LuaExpert", "Konstant", "AdvancedDecompiler", "Shiny"}
...
DefaultSettings = (function()
	...
	return {
		...
		Decompiler = {
			DecompilerFallback = "LuaExpert", --LuaExpert, Konstant, Shiny, AdvancedDecompiler
...
Main = (function()
	...
	Main.InitEnv = function()
		...
		local LuaExpert_http = cloneref and cloneref(game:GetService("HttpService")) or game:GetService("HttpService")

		local function LuaExpertDec(scriptPath)
			...
			local res = env.request({
				...
				Body = LuaExpert_http:JSONEncode({
					script = base64_encode(bytecode)
				})
			})
		...
		env.decompile = function(...)
			...
			elseif typeof(getscriptbytecode) == "function" then
				...
				if fallbackMode == "LuaExpert" then
					return LuaExpertDec(...)
				elseif fallbackMode == "Konstant" then
					return KonstantDec(...)
	...
	local function serialize(val)
		...
		elseif typeof(val) == "Vector3" then
			local serializedVec = {}
			serializedVec.X = val.X
			serializedVec.Y = val.Y
			serializedVec.Z = val.Z
	...
	Main.LoadSettings = function()
		...
		if s and data and data ~= "" then
			...
			if s and decoded then
				...
				-- Deep merge loaded settings into existing Settings, preserving defaults
				local function mergeInto(target, source, defaults)
					for k, defaultVal in pairs(defaults) do
						if type(defaultVal) == "table" and defaultVal._Recurse then
							if type(target[k]) ~= "table" then
								target[k] = {}
							end
							mergeInto(target[k], source[k] or {}, defaultVal)
						else
							target[k] = (source[k] ~= nil) and source[k] or defaultVal
						end
					end
				end
				
				mergeInto(Settings, deserializedData, DefaultSettings)
			else
